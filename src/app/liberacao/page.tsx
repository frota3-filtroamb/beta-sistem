...
const { error } = await supabase.from('movimentacoes').insert({
  ...
  tipo_veiculo: tipoVeiculo, // 'interno' ou 'externo'  
})
      ...
      <div className="flex-1 ml-64">
        ...
        <div className="relative h-44 overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-end pb-5 px-8">
            <div>
              <h1 className="text-2xl font-bold text-white tracking-tight">Liberação Portaria</h1>
              <p className="text-sm text-emerald-300 mt-1">Autorize a saída de veículos internos ou externos</p>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-4xl">
            ...
            <div className="flex gap-3 mb-6">
              <button
                ...
                className={`flex-1 py-3 px-4 rounded-xl text-sm font-semibold transition border ${
                  tipoVeiculo === 'interno'
                    ? 'bg-emerald-500/15 border-emerald-500/40 text-emerald-300'
                    : 'bg-[#0f1c2e] border-white/10 text-slate-400 hover:border-white/20'
                }`}
              >
                🚚 Veículo da Empresa
              </button>
            ...
            <div className="bg-[#0f1c2e] rounded-2xl border border-emerald-500/15 overflow-hidden">
              <div className="px-6 py-4 border-b border-white/5 bg-[#132337]/60 flex items-center justify-between">
                <div>
              ...
              <form onSubmit={handleLiberar} className="p-6 space-y-5">
                ...
                {tipoVeiculo === 'interno' ? (
                  <div className="relative">
                    ...
                    {mostrarListaPlaca && !veiculoSelecionado && buscaPlaca.length >= 1 && (
                      <div className="absolute z-20 w-full mt-1.5 bg-[#132337] border border-emerald-500/25 rounded-xl shadow-2xl max-h-60 overflow-auto">
                        {Array.from(new Map(veiculos.filter(v => v.NR_PLACA?.toLowerCase().includes(buscaPlaca.toLowerCase())).map(v => [v.NR_PLACA, v])).values())
                          .slice(0, 8)
                          .map((v, i) => (
                            <button key={`${v.NR_PLACA}-${i}`} type="button"
                              onClick={() => { setVeiculoSelecionado(v); setBuscaPlaca(v.NR_PLACA); setMostrarListaPlaca(false) }}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="ABC1D23"
                        className="w-full px-4 py-3 bg-[#132337] border border-orange-500/20 rounded-xl text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-400/40 uppercase transition"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Modelo (opcional)</label>
                      <input
                        type="text"
                        value={modeloExterno}
                        onChange={(e) => setModeloExterno(e.target.value)}
                        placeholder="Ex: Fiat Strada"
                        className="w-full px-4 py-3 bg-[#132337] border border-orange-500/20 rounded-xl text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-orange-400/40 transition"
                      />
                    </div>
                  </div>
                )}
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button type="button" onClick={() => setMostrarFormMotorista(!mostrarFormMotorista)}
                      className="text-xs text-emerald-400 hover:text-emerald-300">+ Cadastrar</button>
                  </div>
                  {mostrarFormMotorista && (
                    <div className="mb-2 flex gap-2">
                      <input type="text" value={novoMotorista} onChange={(e) => setNovoMotorista(e.target.value)} placeholder="Nome..."
                        className="flex-1 px-3 py-2 bg-[#132337] border border-emerald-500/20 rounded-lg text-sm text-white" />
                      <button type="button" onClick={() => cadastrarItem('motoristas', novoMotorista, setBuscaMotorista, setMotoristaSelecionado, setMostrarFormMotorista, setNovoMotorista)}
                        className="px-3 py-2 bg-emerald-500 text-[#0a1625] text-sm font-semibold rounded-lg">Salvar</button>
                    </div>
                  )}
                  <input type="text" value={buscaMotorista}
                    onChange={(e) => { setBuscaMotorista(e.target.value); setMotoristaSelecionado(''); setMostrarListaMotorista(true) }}
                  ...
                  {motoristaSelecionado && (
                    <div className="mt-2 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-sm text-emerald-300">✓ {motoristaSelecionado}</div>
                  )}
                  {mostrarListaMotorista && !motoristaSelecionado && (
                    <div className="absolute z-20 w-full mt-1.5 bg-[#132337] border border-emerald-500/25 rounded-xl shadow-2xl max-h-48 overflow-auto">
                      {motoristas.filter(m => m.nome.toLowerCase().includes(buscaMotorista.toLowerCase())).map(m => (
                        <button key={m.id} type="button"
                          onClick={() => { setMotoristaSelecionado(m.nome); setBuscaMotorista(m.nome); setMostrarListaMotorista(false) }}
                          className="w-full text-left px-4 py-2.5 hover:bg-emerald-500/10 text-sm text-slate-200 border-b border-white/5 last:border-0">{m.nome}</button>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="relative">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button type="button" onClick={() => setMostrarFormOrigem(!mostrarFormOrigem)} className="text-xs text-emerald-400">+ Cadastrar</button>
                    </div>
                    {mostrarFormOrigem && (
                      <div className="mb-2 flex gap-2">
                        <input type="text" value={novaOrigem} onChange={(e) => setNovaOrigem(e.target.value)} placeholder="Nova origem..."
                          className="flex-1 px-3 py-2 bg-[#132337] border border-emerald-500/20 rounded-lg text-sm text-white" />
                        <button type="button" onClick={() => cadastrarItem('origens', novaOrigem, setBuscaOrigem, setOrigemSelecionada, setMostrarFormOrigem, setNovaOrigem)}
                          className="px-3 py-2 bg-emerald-500 text-[#0a1625] text-sm font-semibold rounded-lg">Salvar</button>
                      </div>
                    )}
                    <input type="text" value={buscaOrigem}
                      onChange={(e) => { setBuscaOrigem(e.target.value); setOrigemSelecionada(''); setMostrarListaOrigem(true) }}
                    ...
                    {mostrarListaOrigem && (
                      <div className="absolute z-20 w-full mt-1.5 bg-[#132337] border border-emerald-500/25 rounded-xl shadow-2xl max-h-40 overflow-auto">
                        {origens.filter(o => o.nome.toLowerCase().includes(buscaOrigem.toLowerCase())).map(o => (
                          <button key={o.id} type="button"
                            onClick={() => { setOrigemSelecionada(o.nome); setBuscaOrigem(o.nome); setMostrarListaOrigem(false) }}
                            className="w-full text-left px-4 py-2.5 hover:bg-emerald-500/10 text-sm text-slate-200 border-b border-white/5 last:border-0">{o.nome}</button>
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="relative">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button type="button" onClick={() => setMostrarFormDestino(!mostrarFormDestino)} className="text-xs text-emerald-400">+ Cadastrar</button>
                    </div>
                    {mostrarFormDestino && (
                      <div className="mb-2 flex gap-2">
                        <input type="text" value={novoDestino} onChange={(e) => setNovoDestino(e.target.value)} placeholder="Novo destino..."
                          className="flex-1 px-3 py-2 bg-[#132337] border border-emerald-500/20 rounded-lg text-sm text-white" />
                        <button type="button" onClick={() => cadastrarItem('destinos', novoDestino, setBuscaDestino, setDestinoSelecionado, setMostrarFormDestino, setNovoDestino)}
                          className="px-3 py-2 bg-emerald-500 text-[#0a1625] text-sm font-semibold rounded-lg">Salvar</button>
                      </div>
                    )}
                    <input type="text" value={buscaDestino}
                      onChange={(e) => { setBuscaDestino(e.target.value); setDestinoSelecionado(''); setMostrarListaDestino(true) }}
                      onFocus={() => setMostrarListaDestino(true)}
                      placeholder="Buscar destino..."
                      className="w-full px-4 py-3 bg-[#132337] border border-emerald-500/20 rounded-xl text-sm text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-400/40 transition" />
                    {mostrarListaDestino && !destinoSelecionado && (
                      <div className="absolute z-20 w-full mt-1.5 bg-[#132337] border border-emerald-500/25 rounded-xl shadow-2xl max-h-40 overflow-auto">
                        {destinos.filter(d => d.nome.toLowerCase().includes(buscaDestino.toLowerCase())).map(d => (
                          <button key={d.id} type="button"
                            onClick={() => { setDestinoSelecionado(d.nome); setBuscaDestino(d.nome); setMostrarListaDestino(false) }}
                            className="w-full text-left px-4 py-2.5 hover:bg-emerald-500/10 text-sm text-slate-200 border-b border-white/5 last:border-0">{d.nome}</button>
                        ))}
                      </div>
                    )}
                  </div>
                ...
                {mensagem && (
                  <div className={`p-4 rounded-xl text-sm ${mensagem.includes('sucesso') ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/20' : 'bg-red-500/10 text-red-300 border border-red-500/20'}`}>
                    {mensagem}
                  </div>
                )}
              </form>
