...
export default function PortariaPage() {
  ...
  const movimentacoesFiltradas = movimentacoes.filter((m) => {
    ...
    return (
      ...
      m.motorista?.toLowerCase().includes(texto) ||
      m.destino?.toLowerCase().includes(texto) ||
      m.status?.toLowerCase().includes(texto) ||
      String(m.km || '').includes(texto)
    )
  })

  const historicoFiltrado = historico.filter((m) => {
    const texto = busca.toLowerCase()
    return (
      m.placa?.toLowerCase().includes(texto) ||
      m.motorista?.toLowerCase().includes(texto) ||
      ...
      <div className="flex-1 ml-64">
        ...
        <header className="bg-[#0b1f33] border-b border-emerald-500/20 sticky top-0 z-10">
          <div className="px-8 py-5 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-white tracking-tight">Controle</h2>
          ...
          <div className="mb-4 flex items-center gap-3">
            <div className="relative flex-1 max-w-md">
              <input
                ...
                placeholder="Filtrar por placa, motorista, destino, status..."
                            ...
                            <td className="px-5 py-3.5">
                              <div className="flex items-center gap-2">
                                ...
                                {m.tipo_veiculo === 'externo' && (
                                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide bg-orange-500/15 text-orange-300 border border-orange-500/25">
                                    Externo
                                  </span>
                                )}
                              </div>
